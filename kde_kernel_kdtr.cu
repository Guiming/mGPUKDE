...
{	
	size_t n = dAscii.nRows * dAscii.nCols;
	// serial point ID
...
// Kernel density estimation with fixed bandwidth h2 (squared)
__global__ void KernelDesityEstimation_V0(float h2, const SamplePoints dPoints, const AsciiRaster dAscii, float* dWeights)
{
	// serial point ID
	size_t tid = (blockIdx.y * gridDim.y + blockIdx.x) * blockDim.x + threadIdx.x;
...
__global__ void KernelDesityEstimation(float* dHs, const SamplePoints dPoints, const AsciiRaster dAscii, float* dWeights)
{
	...
	float p_x, p_y, p_w, h2;    // x, y coord, weight of point
	float e_w = 1.0;    // edge effect correction weight
...
// Kernel density estimation with adaptive bandwidth (variable bandwidth at each point in dHs)
__global__ void KernelDesityEstimation_V0(float* dHs, const SamplePoints dPoints, const AsciiRaster dAscii, float* dWeights)
{
	// serial point ID
	size_t tid = (blockIdx.y * gridDim.y + blockIdx.x) * blockDim.x + threadIdx.x;
...
__global__ void ReductionSum(float *g_data, int idxg, int div, int N, int iteration, int num_active_items)
{
	...
	int arrIdx = idxg + serial_thread_id * tmp; //pow((double)BLOCK_SIZE, (double)iteration);

	//if (iteration > 1) printf("%d, %f\n", tmp, pow((double)BLOCK_SIZE, (double)iteration));
	...
	if (arrIdx < (idxg + div) && serial_thread_id < num_active_items) {
		...
		//if (iteration >= 2) printf("pow = %f %d %d\n", pow((double)BLOCK_SIZE, (double)iteration), (int)pow((double)BLOCK_SIZE, (double)iteration), tmp);
	}
	else{
	...
	if(thread_id == 0){
		...
		int idx = idxg + (blockIdx.y * gridDim.y + blockIdx.x) * blockDim.x * tmp; //pow((double)blockDim.x, (double)iteration);
		//if (iteration > 1) printf("%d, %f\n", tmp, pow((double)BLOCK_SIZE, (double)iteration));
...
__global__ void ReductionSum_V0(float* g_data, int N, int iteration, int num_active_items)
{
	...
	//unsigned int arrIdx = serial_thread_id * pow((double)BLOCK_SIZE, (double)iteration);
	// avoid using pow() or powf() due to precision issues
	int tmp = 1;
	for (int i = 0; i < iteration; i++) {
		tmp *= BLOCK_SIZE;
	}
	int arrIdx = serial_thread_id * tmp; //pow((double)BLOCK_SIZE, (double)iteration);
	...
	if (thread_id == 0) {
		...
		int idx = (blockIdx.y * gridDim.y + blockIdx.x) * blockDim.x * tmp; //pow((double)blockDim.x, (double)iteration);
		//unsigned long idx = blockDim.x * blockIdx.x * (unsigned long)pow((double)blockDim.x, (double)iteration);

		//if (iteration >= 2) printf("pow = %f %d %d\n", pow((double)blockDim.x, (double)iteration), (int)pow((double)blockDim.x, (double)iteration), tmp);
