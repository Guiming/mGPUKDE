		...
		if (SKIPPARA == 0) {
			...
			*/
			...
			for (int i = 0; i < GPU_N; i++)
			{
				...
				if(sPoints.numberOfPoints < 100000){
					// invoke kernel to do density estimation
					int NBLOCK_K = (dAscii[i].end - dAscii[0].start + BLOCK_SIZE - 1) / BLOCK_SIZE;
					int GRID_SIZE_K = (int)(sqrtf(NBLOCK_K)) + 1;
					dim3 dimGrid_K(GRID_SIZE_K, GRID_SIZE_K);

					KernelDesityEstimation_V0 << <dimGrid_K, BLOCK_SIZE, 0, streams[i] >> > (dHs[i], dPoints[i], dAscii[i], dWeights[i]);
				}
					...
					KernelDesityEstimation << <dimGrid_K, BLOCK_SIZE, 0, streams[i] >> > (dHs[i], dPoints[i], dAscii[i], dWeights[i]);
				}
...
void AllocateDeviceAsciiRaster(AsciiRaster* dAscii, const AsciiRaster hAscii){
	...
	for (int i = 0; i < GPU_N; i++)
	{
		...
		printf("size in AllocateDeviceAsciiRaster %llu\n", size);
